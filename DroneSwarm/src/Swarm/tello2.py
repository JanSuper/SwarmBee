...
class Tello:
    ...
    def __init__(self,
        ...
        if not threads_initialized:
            ...
            #   modify Tello constructor so that it has the option to take in wifi interface name for socket
            #   modify client_socket so that each Tello instance has a different socket instance
            #   it is controlled with, based on the wifi interface it uses
            '''
            import socket
            import time
            drone1 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            drone1.setsockopt(socket.SOL_SOCKET, 2, 'wlp2s0')  # last string has first wifi interface's name
            drone2 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            drone2.setsockopt(socket.SOL_SOCKET, 2, 'wlxf8788c004f0')  # last string has second wifi interface's name
            ...
            if self.wifi_interface != '':
                client_socket.setsockopt(socket.SOL_SOCKET, 2, self.wifi_interface)
    ...
    def send_command_with_return(self, command: str, timeout: int = RESPONSE_TIMEOUT) -> str:
        ...
        client_socket.sendto(command.encode('utf-8'), 0, self.address)
